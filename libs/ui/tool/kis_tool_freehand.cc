      ...
      m_brushResizeCompressor(200, std::bind(&KisToolFreehand::slotDoResizeBrush, this, _1))
{
    ...
    m_quickShape = std::make_unique<KisToolFreehandQuickShape>(this);
}
...
void KisToolFreehand::paint(QPainter &gc, const KoViewConverter &converter)
{
    ...
    m_quickShape->paint(gc, converter);
}
...
{
    if (m_quickShape->handleBeginEditRedirect(event)) {
        return;
    }
    ...
    m_quickShape->startTrackingIfEnabled(event);
...
{
    if (m_quickShape->handleContinueEditRedirect(event)) {
        return;
    }
     ...
     * Actual painting
     */
    m_quickShape->accumulateTrackingPoint(event);
...
{
    if (m_quickShape->handleEndEditRedirect(event)) {
        return;
    }

    CHECK_MODE_SANITY_OR_RETURN(KisTool::PAINT_MODE);

    // QuickShape detection must run BEFORE endStroke() (it reads the live
    // tracking state and the perfect-shape request); the commit (which undoes
    // the just-finished freehand stroke and repaints it as the clean shape)
    // must run AFTER setMode(HOVER), preserving the original interleave.
    m_quickShape->detectShapeOnEnd(event);
    ...
    m_quickShape->commitDetectedShapeAndShowPopup();
}
